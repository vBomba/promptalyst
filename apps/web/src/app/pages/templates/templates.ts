...
export class Templates {
  ...
  protected onTitleChange(id: TemplateId, value: string): void {
    this.titleDraft.update((m) => ({ ...m, [id]: value }));
    this.store.updateTitle(id, this.locale.uiLang(), value.trim());
  }

  protected onBodyChange(id: TemplateId, value: string): void {
    this.bodyDraft.update((m) => ({ ...m, [id]: value }));
    this.store.updateBody(id, this.locale.uiLang(), value);
  }
  ...
  protected openInAnalyzer(id: TemplateId): void {
    ...
    void this.router.navigateByUrl('/analyzer');
  }
