...
@Component({
  ...
  imports: [RouterLink, MatCardModule, FormsModule, SlicePipe],
...
export class HistoryDetail implements OnInit {
  ...
  protected versionLabel(v: PromptVersionStored): string {
    return `v${v.ordinal + 1} · ${new Date(v.createdAt).toLocaleString(this.locale.uiLang())}`;
  }
}
